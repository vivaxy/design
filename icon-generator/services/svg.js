...
function createText({ text, textSize, textColor }) {
...
function createSVG(values) {
  ...
  svg.setAttribute(
    'style',
    `width: ${width}px; height: ${height}px; font-family: system-ui, -apple-system, BlinkMacSystemFont, PingFang SC, Microsoft YaHei, sans-serif;`,
  );

  const rect = createRect(values);
  const $text = createText(values);
  const shadowFilter = createShadowFilter(values);
...
export default {
  init(events) {
    ...
    async function handleInputValueChange(values) {
      ...
      const svgURL = getSVGImageURL($svg.querySelector('svg'));
      ...
      events.emit(E.UPDATE_DOWNLOAD_URL, {
        svgURL,
        pngURL,
    ...
    events.on(E.INPUT_VALUE_CHANGE, handleInputValueChange);
  },
};
