...
function init(events) {
  ...
  function onRenderPreparing(eventId, eventData) {
    eventData.layers.push({
      layerIndex: layerIndexes.CLEAR_CANVAS,
      actions: [
        {
          type: layerActions.FUNCTION,
          func: layerFunctions.CLEAR_RECT,
          params: [0, 0, canvas.width, canvas.height],
        },
      ],
    });
  }

  function onRendering(eventId, eventData) {
    eventData.layers.forEach((layer) => {
      ASSERT(layer.layerIndex, 'Missing layerIndex');
      ...
      layer.actions.forEach((action) => {
        ...
        switch (action.type) {
          ...
          case layerActions.SET_FONT:
            ...
            break;
          default:
            ASSERT(false, 'Unexpected render action');
        }
      });
    });
  }
