...
// set main colors
var lightColor = "46, 204, 113";
var darkColor = "39, 174, 96";
...
// set menu colors
var color = ["rgb(52, 152, 219)", "rgb(231, 76, 60)", "rgb(155, 89, 182)", "rgb(46, 204, 113)", "rgb(26, 188, 156)", "rgb(52, 73, 94)", "rgb(241, 196, 15)", "rgb(230, 126, 34)"];
for (var i in menu) {
    addCss(css, ".menu-open" + i + "", "cursor: pointer; background: " + color[i] + "");
}
// set transform
...
for (i in menu) {
    addCss(css, ".menu" + i, "z-index: -" + i + "; " + transform + ": translate3d(" + (-i * 100) + "px," + (-i * 60) + "px,0) rotate3d(1,2,-1," + (i * deg + 10) + "deg) scale3d(" + (0.6 - i * scale) + "," + (0.6 - i * scale) + "," + (0.6 - i * scale) + ");");
}
// add class to menus
...
// set button click
addCss(css, ".menu-on", "z-index: 1; " + transform + ": translate3d(0,0,0) rotate3d(0,0,0,0deg) scale3d(1,1,1);");
btn.addEventListener(clickEvent, function (e) {
    ...
    menu.forEach(function (e, i) {
        ...
        e.addEventListener(clickEvent, function () {
            e.classList.remove("menu" + i);
            e.classList.add("menu-on" + i);
            setTimeout(function () {
                document.location = xmlDoc.getElementsByTagName("menu")[0].getElementsByTagName("link")[i].childNodes[0].nodeValue;
            }, 500);
        });
    });
    addScroll();
